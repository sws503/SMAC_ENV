...
from pysc2.lib import protocol, actions, features

from s2clientprotocol import common_pb2 as sc_common
from s2clientprotocol import sc2api_pb2 as sc_pb
from s2clientprotocol import raw_pb2 as r_pb
from s2clientprotocol import debug_pb2 as d_pb
from s2clientprotocol import spatial_pb2 as s_pb
...
class StarCraft2Env(MultiAgentEnv):
    ...
    def unit_max_shield(self, unit):
        ...
        if unit.unit_type == 77 or unit.unit_type == self.sentry_id:
            return 140 # Protoss's Sentry
        if unit.unit_type == 75 or unit.unit_type == self.hightemplar_id:
            return 140 # Protoss's High Templar
        if unit.unit_type == 84 or unit.unit_type == self.probe_id:
            return 20 # Protoss's Probe
        if unit.unit_type == 59 or unit.unit_type == self.nexus_id:
            return 1000 # Protoss's Probe
        ...
        for e_id, e_unit in self.enemies.items():
            ...
            if e_unit.health > 0:
                ...
                if self.shield_bits_enemy > 0:
                    ...
                    ind += 1
                print("enemy_state is: ", enemy_state)
                if self.unit_type_bits > 0:
                    type_id = self.get_unit_type_id(e_unit, False)
        ...
        if ally:  # use new SC2 unit types
            ...
            if self.map_type == "stalkers_and_zealots":
                ...
                if unit.unit_type == 51:
                    ...
                    type_id = 2
            elif self.map_type == "Sentry_HT_Woong":
                if unit.unit_type == 75:
                    type_id = 0
